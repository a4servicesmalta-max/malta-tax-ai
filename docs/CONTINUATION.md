...
6d. **AI auth: Max-first, API-key fallback (added 2026-07-03):** `src/ai-auth.ts` centralises Claude auth for both AI features. Prefers a Claude Max/Pro OAuth token (`CLAUDE_CODE_OAUTH_TOKEN`/`ANTHROPIC_AUTH_TOKEN`, from `claude setup-token`) — Bearer + `anthropic-beta: oauth-2025-04-20` + Claude Code identity prepended to the system prompt (mirrors the portal factory `_reint_be/.../anthropic.config.ts`) — and **automatically falls back to `ANTHROPIC_API_KEY` on a 429 (Max maxed out) or 401/403 (token unusable)**. `runWithFallback()` is the unit-tested core; `callAnthropic()` wires the real SDK clients; `isAiConfigured()` gates both callers. Loaded via `tsx --env-file-if-exists=.env`. Live-verified: dummy OAuth token → 401 → fell back to API key → real Fable 5 reply; live reasonableness review returned 5 correct findings.
6c. **AI reasonableness review (added 2026-07-03, ADVISORY):** `POST /api/session/:id/review` → `reasonablenessReview()` (`src/reasonableness-review.ts`) — an optional AI second opinion on the draft computation that flags missing add-backs, off-looking CA/loss figures, unusual items, mapping smells. Reuses the ai-mapper plumbing (env-gated on `ANTHROPIC_API_KEY`, `claude-fable-5`, `sanitizeName` now exported+shared, test seam, graceful fallback). **Golden-rule-safe: it returns only prose findings {severity, message}, never a figure, and never gates Generate.** Unconfigured/failed → `{available:false, findings:[], note}`. UI: "Run reasonableness review (AI)" button appears once the computation is prepared; findings show in an advisory panel; edits clear it. NOT written into the filed summary.html (pre-filing advisory only).
